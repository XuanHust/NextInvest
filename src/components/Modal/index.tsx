...
const Modal = ({
  ...
  ReactDOM.createPortal(
    <React.Fragment>
      {open && (
        <div className="fixed w-screen left-0 bottom-0 top-0 flex justify-center items-center z-[1000">
          <div
            className="bg-black/30 absolute top-0 left-0 bottom-0 h-full w-full z-50"
          ...
          >
            <div className={`bg-white rounded-lg shadow-md z-[60] ${className}`}>
              <div className="flex justify-between items-center p-3">
                ...
                <XCircle
                  className="cusor-pointer transition-all hover:scale-90"
                  onClick={() => handleCancel && handleCancel()}
                />
              </div>
              ...
              <hr className="border-black/[0.08]" />
              <div className={`${display ? "hidden" : "flex"} justify-center gap-2 items-center p-3`}>
                <button
